    ...
    fun vendorHint(context: Context): String? {
        ...
        return when {
            manufacturer.contains("xiaomi") || manufacturer.contains("redmi") ->
                "小米/红米手机：请在「安全中心 → 应用管理 → 权限 → 自启动」中允许收纳管家自启动，否则后台提醒可能失效。"
            manufacturer.contains("huawei") || manufacturer.contains("honor") ->
                "华为/荣耀手机：请在「手机管家 → 应用启动管理」中关闭收纳管家的自动管理，并允许自启动。"
            manufacturer.contains("oppo") || manufacturer.contains("realme") ||
                manufacturer.contains("oneplus") ->
                "OPPO/一加/realme 手机：请在「设置 → 应用 → 收纳管家 → 允许自启动」中开启，并允许后台运行。"
            manufacturer.contains("vivo") || manufacturer.contains("iqoo") ->
                "vivo/iQOO 手机：请在「i管家 → 应用管理 → 权限管理 → 自启动」中允许收纳管家自启动。"
            manufacturer.contains("meizu") ->
                "魅族手机：请在「设置 → 应用管理 → 收纳管家 → 权限管理」允许通知与自启动，并到「手机管家 → 应用管理 → 收纳管家」允许后台运行，否则提醒可能被拦截。"
            else -> null
        }
    }
